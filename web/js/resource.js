...
Builder.prototype.getResult = function () {
    ...
    switch (module) {
		...
		case "overall":
			if ($("div.choose").length > 0) {
                var v = $("#r_overall input[type='hidden']");
                if (v.length == 0) {
                    alert("请选择全景");
                } else {
                    data = {
		...
		case "album":
			if ($("div.choose").length > 0) {
                var v = $("#r_album input[type='hidden']");
                if (v.length == 0) {
                    alert("请选择相册");
                } else {
                    data = {
		...
		case "meeting":
			if ($("div.choose").length > 0) {
                var v = $("#r_meeting input[type='hidden']");
                if (v.length == 0) {
                    alert("请选择微会议");
                } else {
                    data = {
		...
		case "store":
			if ($("div.choose").length > 0) {
                var v = $("#r_store input[type='hidden']");
                if (v.length == 0) {
                    alert("请选择微门店");
                } else {
                    data = {
		...
		case "reserve":
			if ($("div.choose").length > 0) {
                var v = $("#r_reserve input[type='hidden']");
                if (v.length == 0) {
                    alert("请选择微预约");
                } else {
                    data = {
		...
		case "message":
			if ($("div.choose").length > 0) {
                var v = $("#r_message input[type='hidden']");
                if (v.length == 0) {
                    alert("请选择微留言");
                } else {
                    data = {
		...
		case "survey":
			if ($("div.choose").length > 0) {
                var v = $("#r_survey input[type='hidden']");
                if (v.length == 0) {
                    alert("请选择微调研");
                } else {
                    data = {
        ...
        case "activity":
            ...
            if ($selected.length == 0) {
                ...
                if (t == $selected.attr("data-type")) {
                    ...
                    alert("网络异常，请尝试刷新本页面再进行操作！"+$selected.attr("data-type"))
                }
            }
            break;
        case "business":
    ...
    if (!loaded) {
        switch (module) {
            case "article":
                ...
                break;
            case "overall":
                if($("div.overall_choose").length > 0) {
                    var v = $("#r_overall input[type='hidden']");
                    if (v.length == 0){
                        _overall_choose_init();
                    }
                };
                break;
            case  "album":
				if($("div.album_choose").length > 0) {
                    var v = $("#r_album input[type='hidden']");
                    if (v.length == 0){
                        _album_choose_init();
                    }
                ...
                break;
            case  "meeting":
				if($("div.meeting_choose").length > 0) {
                    var v = $("#r_meeting input[type='hidden']");
                    if (v.length == 0){
                        _meeting_choose_init();
                    }
                };
                break;
			case "store":
				if($("div.store_choose").length > 0) {
                    var v = $("#r_store input[type='hidden']");
                    if (v.length == 0){
                        _store_choose_init();
                    }
                ...
                break;
			case "reserve":
				if($("div.reserve_choose").length > 0) {
                    var v = $("#r_reserve input[type='hidden']");
                    if (v.length == 0){
                        _reserve_choose_init();
                    }
                };
                break;
			case "message":
				if($("div.message_choose").length > 0) {
                    var v = $("#r_message input[type='hidden']");
                    if (v.length == 0){
                        _message_choose_init();
                    }
                };
                break;
			case "survey":
				if($("div.survey_choose").length > 0) {
                    var v = $("#r_survey input[type='hidden']");
                    if (v.length == 0){
                        _survey_choose_init();
                    }
...
Builder.prototype._loadAct = function () {
	...
	if (100 != t) { // 圣诞活动特殊处理 @annchen
		$.post("/activity/uc/activity/getactivity", {
			type: t
		}, function (result) {
			if (result.success) {
				var data = result.data;
				...
				if (data.length > 0) {
					...
					for (var i = 0; i < data.length; i++) {
						var item = data[i];
						cont.push('<tr data-type="' + t + '" data-id="' + item.id + '" class="data-row">');
						cont.push('<td class="with-checkbox"><input type="radio" name="WeisiteEvent[active_id]" class="act-sel" value="' + item.id + '"/></td>');
						cont.push('<td class="title">' + item.name + "</td>");
						cont.push('<td class="keyword">' + item.keyword + "</td>");
						cont.push("<td>" + item.start_time + "~" + item.end_time + "</td>");
						cont.push("</tr>")
					}
...
Builder.prototype._loadBaidu_map = function () {
    ...
    if (data) {
        ...
        var point = new BMap.Point(116.385292, 39.977647);
        doit(point);
...
window.ICON = function () {
    ...
    var list_usually = ["icon-location-arrow", "icon-map-marker","icon-phone","icon-phone-sign","icon-microphone","icon-food","icon-reply","icon-mail-forward","icon-shopping-cart","icon-truck","icon-coffee","icon-glass","icon-home","icon-credit-card"];
    ...
    $.each(list_usually, function (k, v) {
        s3 += tmp.format(v);
    })
   $("#ico_hot").html(ul.format(s));
    $("#ico_all").html(ul.format(s2));
    $("#ico_usually").html(ul.format(s3));
	$(".icons-cont").hide();

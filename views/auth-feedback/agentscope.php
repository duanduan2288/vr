            ...
            <div class="portlet-body">
                <table class="table table-striped table-hover table-bordered table-advance">
                    <tbody>
                        ...
                        <tr>
                            <td align="right">已管理代理商&nbsp;:</td>
                            <td>
                                <?php
                                    $str1 = '';
                                    foreach ($employee_agents as $k => $v) {
                                        if ($k!=count($employee_agents)-1) {
                                            $str1.= Service::get_agent_name_by_code($v).'<span style="color:red;">&nbsp;|&nbsp;</span>';
                                        }else{
                                            $str1.= Service::get_agent_name_by_code($v);
                                        }
                                    }
                                    echo trim($str1,',');
                                ?>
                ...
                <form role="form" id="myform" class="form-horizontal validate" enctype="multipart/form-data" method="post">
                    ...
                    <div class="form-body">
                        <div class="form-group">
                            <div class="col-md-5">
                                <div class="tishi" style="color:red;">未管理代理商</div>
                                <select class="select" id="select1" multiple="multiple">
                                    <?php foreach ($registrars as $v) : ?>
                                        <option value="<?php echo $v; ?>">
                                            <?php
                                               echo Service::get_agent_name_by_code($v);
                                            ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="col-md-5">
                                <div class="tishi" style="color:green;">已管理代理商</div>
                                <select class="select" name="agent_codes" id="select2" multiple="multiple">
        ...
        if(saveflag==false){
            $('#saveform').click(function(){
                ...
                if (confirm('确定执行此操作吗')) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '/auth-feedback/agent',
                        data: {'user_id':user_id,'agent_codes':arr},
                        dataType:'json',
                        success: function (r) {
                            layer.close(load);
                            saveflag = false;
                            if('err'==r.info){
                                ...
                                common_layer(r.data,'/auth-feedback/userlist');
                            }
                        },
                        error: function () {
                            layer.close(load);
